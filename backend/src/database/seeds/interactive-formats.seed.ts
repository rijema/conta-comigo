...
const formats: Format[] = [
  ...
  { id: 'visual_addition', niche: 'addition', skills: ['EF01MA06'], type: 'quiz', prompt: 'Junte os dois grupos. Quantos dados há no total?', levels: [2, 4], build: (v) => choice(visual(v, v - 1), [{ text: String(v + v - 1), pictogramConceptId: n(v + v - 1), pictogramConceptIds: [...Array(v + v - 1)].map(() => 'library.die') }, ...numbers([v + v - 2])], String(v + v - 1)) },
  { id: 'visual_subtraction', niche: 'subtraction', skills: ['EF01MA08'], type: 'quiz', prompt: 'Separe um dado do grupo. Quantos restam?', levels: [3, 6], build: (v) => choice(visual(v), [{ text: String(v - 1), pictogramConceptId: n(v - 1), pictogramConceptIds: [...Array(v - 1)].map(() => 'library.die') }, ...numbers([v - 2, v])], String(v - 1)) },
